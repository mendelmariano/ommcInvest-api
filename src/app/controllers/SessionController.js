...
class SessionController {
    ...
    async storeFab(req, res) {
        try {
          ...
          if (!ldapUser) {
            const userLocal = await User.findOne({ where: { email: req.body.email } });
      
            if (!userLocal || !(await userLocal.checkPassword(req.body.password))) {
            ...
            return res.json({
              ...
              token: jwt.sign({ id, user: userLocal }, authConfig.secret, {
                expiresIn: authConfig.expiresIn,
              }),
            });
          }
          ...
          const hashMatches = await bcrypt.compare(req.body.password, user.password_hash);
          ...
          return res.status(401).json({ message: 'Token inválido', erro: error });
        }
      }
